...
export class StripeService {
  ...
  async checkPaymentStatus(paymentLinkId: string): Promise<{
    ...
    try {
      ...
      const failedSession = sessions.data.find(
    ...
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        try {
          ...
          console.warn(
            `Could not retrieve charges for payment intent ${paymentIntentId}:`,
            chargeError.message,
          );
        }
      }
      ...
      console.error(
        `Error retrieving payment details for ${paymentIntentId}:`,
        error.message,
      );
      return {
        paymentIntentId,
