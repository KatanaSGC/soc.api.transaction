...
export class GetPaymentStatusHandler
  implements IQueryHandler<GetPaymentStatusQuery>
{
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      const payment = await this.transactionPaymentRepository
        ...
        .where('transaction.TransactionCode = :transactionCode', {
      ...
      return {
        ...
        data: {
          ...
          paymentId:
            payment.StripePaymentLinkId || payment.StripeCheckoutSessionId,
        },
        error: null,
      };
    } catch (error) {
