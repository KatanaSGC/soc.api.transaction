...
export class GeneratePaymentHandler
  implements ICommandHandler<GeneratePaymentCommand>
{
  constructor(
  ...
  async execute(
    command: GeneratePaymentCommand,
    ...
    if (findTransactionPayment) {
      console.log(
        'Payment link already exists for transaction:',
    ...
    console.log(
      'Generating payment link for transaction:',
      command.TransactionCode,
    );
