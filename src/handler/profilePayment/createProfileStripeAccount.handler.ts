...
export class CreateProfileStripeAccountHandler
  implements ICommandHandler<CreateProfileStripeAccountCommand>
{
  constructor(
  ...
  async execute(
    command: CreateProfileStripeAccountCommand,
    ...
    if (findStripeAccount) {
      apiResponse.status = ResponseCode.ERROR;
      apiResponse.message =
        'Ya existe una cuenta de Stripe asociada a este perfil.';
      return apiResponse;
    }
