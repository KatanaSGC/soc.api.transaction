...
export class ChargeTransaccionHandler
  implements ICommandHandler<ChargeTransaccionCommand>
{
  ...
  async execute(
    command: ChargeTransaccionCommand,
    ...
    try {
      ...
      if (!findTransaction) {
        response.message =
          'No se encontró la transacción, verifique el código de transacción.';
        return response;
      }

      const findTransactionPayment =
        await this.transactionPaymentRepository.findOneBy({
      ...
      const findTransactionPaymentState =
        await this.transactionPaymentStateRepository.findOneBy({
          Id: findTransactionPayment.TransactionPaymentStateId,
        });

      if (
        findTransactionPaymentState?.TransactionPaymentStateCode !== 'TPS-02'
      ) {
        response.message =
          'El pago no está en estado completado para poder transferir al vendedor.';
        return response;
      }
      ...
      const sellerStripeAccount =
        await this.profileStripeAccountRepository.findOneBy({
      ...
      if (!sellerStripeAccount) {
        response.data = null;
        response.message =
          'El vendedor no tiene una cuenta de Stripe configurada.';
        response.status = ResponseCode.ERROR;
        return response;
      }
      ...
      const transfer = await this.stripeService.createTransfer(
        ...
        `Pago por transacción ${findTransaction.TransactionCode} - Comisión plataforma: ${this.PLATFORM_COMMISSION_PERCENTAGE * 100}%`,
        `transaction_${findTransaction.TransactionCode}`,
      );
      ...
      const findTransactionPaymentStateCompleted =
        await this.transactionPaymentStateRepository.findOneBy({
      ...
      findTransactionPayment.TransactionPaymentStateId =
        findTransactionPaymentStateCompleted.Id;
  ...
  private calculateSellerPayment(originalAmount: number): {
    ...
    const platformCommission =
      originalAmount * this.PLATFORM_COMMISSION_PERCENTAGE;
