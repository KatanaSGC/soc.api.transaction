...
export class CompleteTransactionHandler
  implements ICommandHandler<CompleteTransactionCommand>
{
  constructor(
  ...
  async execute(
    ...
    const findTransactionPayment =
      await this.transactionPaymentRepository.findOne({
        where: {
          TransactionCode: command.TransactionCode,
        },
        order: {
    ...
    if (!findTransactionPayment) {
      response.status = ResponseCode.ERROR;
      response.message =
        'No se encontro información de pago para la transacción.';
      response.data = null;
    }

    if (
      findTransactionPayment!.TransactionUnlockCode !==
      command.TransactionUnlockCode
    ) {
      response.status = ResponseCode.ERROR;
      response.message =
        'El código de desbloqueo de la transacción no es correcto.';
      response.data = null;
      return response;
    }
    ...
    const findTransactionSeller = findTransactions.find(
      (t) => t.IsBuyTransaction === false,
    );
    ...
    const buyTransaction = findTransactions.find(
      (t) => t.IsBuyTransaction === true,
    );
