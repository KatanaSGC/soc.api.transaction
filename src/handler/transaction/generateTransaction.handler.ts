...
export class GenerateTransactionHandler
  implements ICommandHandler<GenerateTransactionCommand>
{
  constructor(
  ...
  async execute(
    command: GenerateTransactionCommand,
    ...
    const sellers = [
      ...new Set(
        shoppingCartDetails.map((shopping) => shopping.SellerUsername),
      ),
    ];

    sellers.forEach(async (seller) => {
      await this.createTransactionSeller(
        seller,
    ...
    const paymentResponse = await this.commandBus.execute(
  ...
  private async createTransactionSeller(
    sellerUsername: string,
    transactionCode: string,
    buyerUsername: string,
  ): Promise<void> {
    const findSellerItems = shoppingCartDetails.filter(
      (item) => item.SellerUsername === sellerUsername,
    );
    if (!findSellerItems || findSellerItems.length === 0) return;

    const productIds = [
      ...new Set(findSellerItems.map((item) => item.ProductId)),
    ];
    let sellerAmount = 0;

    productIds.forEach(async (item) => {
      const filterProducts = findSellerItems.filter(
        (product) => product.ProductId === item,
      );
      const product = findSellerItems.find(
        (product) => product.ProductId === item,
      );
      const totalUnits = filterProducts.reduce(
  ...
  private async findShoppingCartDetail(
    profileId: number,
