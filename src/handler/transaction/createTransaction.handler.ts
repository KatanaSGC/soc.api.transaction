  ...
  implements ICommandHandler<CreateTransactionCommand>
{
  ...
  async execute(
    ...
    try {
      ...
      const [isValidUsernames, usernameMessage] = await this.validateUsernames(
      ...
      const [isValidProduct, productMessage] =
      ...
      const findTransactionState =
        await this.transactionStateRepository.findOneBy({
          TransactionStateCode: 'TS-01',
        });
      ...
      console.log(
        'Createating transaction with profileProductPrice:',
        profileProductPrice,
      );
      ...
      generatePaymentCommand.TransactionCode =
        createTransaction.TransactionCode;

      console.log('Generating payment with command:', generatePaymentCommand);

      const generatePayment = await this.commandBus.execute(
        generatePaymentCommand,
      );
      if (generatePayment.status !== ResponseCode.SUCCESS) {
        response.status = ResponseCode.ERROR;
        response.message =
          'Transacion creada exitosamente, pero no se pudo generar el enlace de pago, puedes intentar nuevamente en la sección de transacciones.';
        return response;
      }
  ...
  async validateProduct(
    ...
    profileProduct = await this.profileProductRepository.findOneBy({
      Id: command.ProfileProductId,
    });
    if (!profileProduct) return [false, 'Profile product does not exist'];

    if (profileProduct.Units < command.ProductUnits)
      return [false, 'Insufficient units in profile product'];

    product = await this.productRepository.findOneBy({
      Id: profileProduct.ProductId,
    });
