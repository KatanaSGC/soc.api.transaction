...
export class PayTransactionHandler
  implements ICommandHandler<PayTransactionCommand>
{
  constructor(
  ...
  async execute(command: PayTransactionCommand): Promise<ApiResponse<boolean>> {
    ...
    const [isValid, message] = await this.validatePaymentAndLoadData(
    ...
    await this.removeProductsFromInventoryAndCreateTransaction(
      command.TransactionCode,
    );

    transactionPayment!.TransactionPaymentStateId =
      nextTransactionPaymentState!.Id;
    transactionPayment!.StripePaymentIntentId =
  ...
  private async validatePaymentAndLoadData(
    ...
    if (!transaction)
      return [
        true,
        'No se encontro la transaccion, verifique el codigo de transaccion.',
      ];
    ...
    checkPaymentStatus = await this.stripeService.checkPaymentStatus(
      transactionPayment.StripePaymentLinkId,
    );
    ...
    firstTransactionPaymentState =
      await this.transactionPaymentStateRepository.findOneBy({
        TransactionPaymentStateCode: 'TPS-01',
      });

    nextTransactionPaymentState =
      await this.transactionPaymentStateRepository.findOneBy({
  ...
  private async removeProductsFromInventoryAndCreateTransaction(
    transactionCode: string,
  ) {
    ...
    const selectUsernames = [
      ...new Set(findFlowTransaction.map((t) => t.Username)),
    ];
    ...
    for (const username of selectUsernames) {
      const transactionSeller = findFlowTransaction.find(
      ...
      const shoppingCartDetailsByUser = shoppingCartDetails.filter(
        (scd) => scd.ProfileId === findProfile?.Id,
      );
