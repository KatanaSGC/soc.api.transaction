...
export class FindAllTransactionHandler
  implements IQueryHandler<FindAllTransactionQuery>
{
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      const transactionsCodes = findTransactions.map(
      ...
      const profiles = findAllTransactions.map(
      ...
      const findAllTransactionPayments =
        await this.transactionPaymentRepository.find({
          where: {
            TransactionCode: In(transactionsCodes),
          },
        });
      ...
      findTransactions.forEach((transaction) => {
        const findCloseState = transactionStates.find(
          (state) => state.TransactionStateCode === 'TS-03',
        );

        const findLastTransaction = findAllTransactions.find(
          (t) =>
        ...
        const findSellTransaction = findAllTransactions.find(
          (t) =>
            t.TransactionCode === transaction.TransactionCode &&
            t.IsBuyTransaction === false,
        );
        const findBuyTransaction = findAllTransactions.find(
          (t) =>
            t.TransactionCode === transaction.TransactionCode &&
            t.IsBuyTransaction === true,
        );

        const findBuyerProfile = findProfiles.find(
          (p) => p.Identify === findBuyTransaction!.Username,
        );
        const findSellerProfile = findProfiles.find(
          (p) => p.Identify === findSellTransaction!.Username,
        );

        const transactionDetail = new TransactionDetailDto();
        transactionDetail.SellerUsername = findSellTransaction
          ? findSellTransaction.Username
          : '';
        ...
        transactionDetail.TransactionState =
          transactionStates.find(
            (state) => state.Id === transaction.TransactionStateId,
          )?.Description || '';
        transactionDetail.TransactionStateCode =
          transactionStates.find(
            (state) => state.Id === transaction.TransactionStateId,
          )?.TransactionStateCode || '';

        const payment = findAllTransactionPayments.find(
          (payment) => payment.TransactionCode === transaction.TransactionCode,
        );
        ...
        transactionDetail.TransactionUnlockCode =
          payment?.TransactionUnlockCode || '';
