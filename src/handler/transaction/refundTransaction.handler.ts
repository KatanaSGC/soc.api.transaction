...
export class RefundPaymentHandler
  implements ICommandHandler<RefundTransactionCommand>
{
  ...
  async execute(
    ...
    try {
      ...
      if (
        ...
        response.message =
          'El pago no está en estado completado para poder realizar el reembolso.';
        return response;
      }

      if (!findTransactionPayment.StripePaymentIntentId) {
        response.message =
          'No se encontró el ID del PaymentIntent de Stripe para realizar el reembolso.';
        return response;
      }
  ...
  private async validateRefundStatus(refund: any): Promise<{
    ...
    if (pendingStatuses.includes(refund.status)) {
      ...
      return {
        isSuccessful: false,
        errorMessage:
          'El reembolso está pendiente de procesamiento. Se completará en las próximas horas.',
        shouldRetry: true,
      };
    }
